...
<div class="max-w-7xl mx-auto">
    ...
    <div class="grid {{ $gridClass }} gap-5 mb-8">
        @if(empty($selectedRole))
            {{-- SEMUA ROLE: 5 card --}}
            {{-- Card akan ditampilkan di controller --}}
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="9" class="text-center py-8 text-gray-400">Tidak ada data pekerja sawit</td></td>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="8" class="text-center py-8 text-gray-400">Tidak ada data security</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="8" class="text-center py-8 text-gray-400">Tidak ada data cleaning</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr><td colspan="6" class="text-center py-8 text-gray-400">Tidak ada data kantoran</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <div class="bg-white rounded-2xl max-w-md w-full max-h-[80vh] overflow-y-auto">
        ...
        <div class="p-6 space-y-4">
            ...
            <div class="border-t pt-3">
                ...
                <div class="flex flex-wrap gap-4" id="absenFotoContainer"></div>
            </div>
        </div>
    </div>
</div>
...
{{-- POPUP ZOOM FOTO --}}
<div id="zoomFotoPopup" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-[60] p-4">
    <div class="relative max-w-2xl w-full">
        <button onclick="closeZoomFoto()" class="absolute -top-10 right-0 text-white hover:text-gray-300 text-3xl">&times;</button>
        <img id="zoomFotoImg" src="" class="w-full rounded-xl shadow-2xl">
    </div>
</div>
...
function showBuktiAbsenPopup(data) {
    ...
    document.getElementById('absenPopupSubtitle').innerHTML = formattedDate;
    
    // Format Waktu Check In (jam:menit:detik)
    let checkInText = '-';
    if (data.check_in) {
        try {
            let checkInDate = new Date(data.check_in);
            if (!isNaN(checkInDate.getTime())) {
                checkInText = checkInDate.toLocaleTimeString('id-ID', {
                    hour: '2-digit',
                    minute: '2-digit',
                    second: '2-digit',
                    hour12: false
    ...
    // Format Waktu Check Out (jam:menit:detik)
    let checkOutText = '-';
    if (data.check_out) {
        try {
            let checkOutDate = new Date(data.check_out);
            if (!isNaN(checkOutDate.getTime())) {
                checkOutText = checkOutDate.toLocaleTimeString('id-ID', {
                    hour: '2-digit',
                    minute: '2-digit',
                    second: '2-digit',
                    hour12: false
    ...
    document.getElementById('absenCheckIn').innerText = checkInText;
    document.getElementById('absenCheckOut').innerText = checkOutText;
    ...
    if (data.foto_checkin) {
        fotoContainer.innerHTML += `
            <div onclick="openZoomFoto('${data.foto_checkin}')" class="cursor-pointer group">
                <div class="w-24 h-24 bg-gray-100 rounded-xl overflow-hidden border-2 border-[#2c5e4e]/20 hover:border-[#2c5e4e] transition-all">
                    <img src="${data.foto_checkin}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-200" alt="Foto Check In">
                </div>
                <p class="text-xs text-center text-gray-500 mt-1">Check In</p>
            </div>
        `;
    }
    if (data.foto_checkout) {
        fotoContainer.innerHTML += `
            <div onclick="openZoomFoto('${data.foto_checkout}')" class="cursor-pointer group">
                <div class="w-24 h-24 bg-gray-100 rounded-xl overflow-hidden border-2 border-blue-200 hover:border-blue-500 transition-all">
                    <img src="${data.foto_checkout}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-200" alt="Foto Check Out">
                </div>
                <p class="text-xs text-center text-gray-500 mt-1">Check Out</p>
            </div>
        `;
    }
...
function showBuktiKerjaPopup(data) {
    ...
    if (data.type === 'user') {
        ...
        let patroliHtml = '';
        data.data.patroli.forEach((p, idx) => {
            patroliHtml += `
                <div class="bg-gray-50 rounded-xl p-3 mb-3">
                    ...
                    ${p.foto ? `<div class="mt-2 ml-6"><div onclick="openZoomFoto('${p.foto}')" class="cursor-pointer inline-block"><img src="${p.foto}" class="w-20 h-20 object-cover rounded-lg border-2 border-purple-200 hover:border-purple-500 transition-all"></div></div>` : ''}
                </div>
            `;
        });
        ...
        let kinerjaHtml = '';
        data.data.kinerja.forEach((k, idx) => {
            kinerjaHtml += `
                <div class="bg-gray-50 rounded-xl p-3 mb-3">
                    <p class="font-semibold text-gray-800 flex items-center gap-2">
                    ...
                    ${k.foto ? `<div class="mt-2 ml-6"><div onclick="openZoomFoto('${k.foto}')" class="cursor-pointer inline-block"><img src="${k.foto}" class="w-20 h-20 object-cover rounded-lg border-2 border-orange-200 hover:border-orange-500 transition-all"></div></div>` : ''}
                </div>
            `;
        });
...
function openZoomFoto(fotoUrl) {
    const zoomModal = document.getElementById('zoomFotoPopup');
    const zoomImg = document.getElementById('zoomFotoImg');
...
document.getElementById('zoomFotoPopup').addEventListener('click', function(e) {
    if (e.target === this) closeZoomFoto();
});
</script>
@endsection